...
/** Builds the purchase-confirmation caption: `whatsappName` (best-effort,
 * the recipient's own WhatsApp display name) greets them personally when
 * known; `buyerName` (from the caller's own records) and `businessName`
 * (from Minha Empresa) always appear, so it's never ambiguous who bought
 * what even when the WhatsApp profile name belongs to someone else (shared
 * number, family member, ...). */
function buildPurchaseConfirmationMessage(input: {
  whatsappName: string | null;
  buyerName: string;
  businessName: string;
}): string {
  const greeting = input.whatsappName ? `Olá, ${input.whatsappName}!` : "Olá!";
  return `${greeting} O pedido comprado em ${input.businessName}, por ${input.buyerName}, foi aprovado com sucesso.`;
}
 ...
 * The caption text itself is built by us (see buildPurchaseConfirmationMessage)
 * — the caller only supplies `buyerName`, never free text.
 */
